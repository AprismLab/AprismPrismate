...
## Extension Pack 边界（v26.13+）

Prismate 只加载 Aprism 原生 `.aje` 模组，故意忽略宿主加载器 `mods/`
目录中的 Aprism Extension Pack (`.aep`)。扩展包用于增强 Aprism agent，
由 agent runtime 从 `aprism-extensions/` 加载，并非 Prismate 的执行对象。
这同样适用于 AprismWarp 编辑器引入的可选
`aprismwarp.editor.json` catalog（`aprismwarp.aep-editor/v1`）：它只由编辑器
读取，Aprism 与 Prismate 都不执行它。`.aep` 与 `.aje` 并存时不会产生发现
警告或加载尝试。
