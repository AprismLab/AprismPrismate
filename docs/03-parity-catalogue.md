...
| Content binding (items/blocks → live registries, v26.7/v26.19) | **PROVEN-BRIDGE** | NONE | v26.19-A2 end-to-end proof: Prismate-loaded .aje mod registered a real Item into BuiltInRegistries.ITEM purely via reflection (Registries.ITEM key → ResourceKey.create → Properties.setId → Registry.register → readback OK), with zero loader-specific imports and zero compile-time MC dependency. A loader-neutral ReflectiveContentBinder seam is now implementable. NeoForge reachability pending a version-compatible instance. |
